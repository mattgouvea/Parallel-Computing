...
//função que faz o cálculo da integral recursivamente
double quad_adaptativa(struct parametro p);

double inicio, fim; //variáveis para fazer a medição do tempo
int calculando=0; //variável que indica quantas threads estão ainda calculando o valor de um retângulo
...
//função que pega o próximo elemento da pilha
struct parametro pega_proximo_elemento(double *ret, double soma_local) {

    while(esta_vazia() && calculando) { //se a pilha tiver vazia e tiver alguém calculando, espera o calculo terminar
        pthread_cond_wait(&thread_executando, &mutex); //espera na condição de que existe thread executando
        if(esta_vazia() && !calculando) { //se a pilha estiver vazia e não tiver ninguém calculando, retorna
...
void *calcula_thread(void *args) {
    ...
    ret = malloc(sizeof(double)); if (ret == NULL) {printf("Erro no malloc\n"); exit(-1);}  
    struct parametro p; //struct com os argumentos necessários pra chamar a função
    double soma_local = 0;
    pthread_mutex_lock(&mutex);
    while(!esta_vazia() || calculando) { //enquanto a pilha tiver elemento ou alguma thread ainda estiver calculando

        p = pega_proximo_elemento(ret, soma_local); //pega o próximo elemento na pilha, caso contrário retorna a soma local
        pthread_mutex_unlock(&mutex); 
...
double quad_adaptativa (struct parametro p) {
    ...
    //calcula valor da função nos pontos médios
    fm = p.f(m);
    fm1 = p.f(m1);
    fm2 = p.f(m2);
    ...
    if (erro <= p.erro_max) {
        ...
        pthread_mutex_lock(&mutex);
        push_arr(p1); //põe um retângulo na pilha
...
int main (int argc, char *argv[]) {
    ...
    double *retorno; //variável para guardar a soma dos retornos das threads
    ...
    //atribui os argumentos à estrutura usada para acomodar as entradas do usuário
    p_inicial = atribui_parametros(atoi(argv[1]), atof(argv[2]), atof(argv[3]), atof(argv[4]));
    push_arr(p_inicial); //põe o primeiro elemento na pilha
